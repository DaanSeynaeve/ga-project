...
function NewChromSub = recombin_path(REC_F, Chrom, RecOpt, Dist, SUBPOP);
...
% Select individuals of one subpopulation and call low level function
   ...
   for irun = 1:SUBPOP
      ...
      number_individuals = size(ChromSub,1);
      if rem(number_individuals,2)~=0
	   number_individuals=number_individuals-1;
      end
      for row=1:2:number_individuals
        parents = [ChromSub(row,:); ChromSub(row+1,:)];
        if strcmp(REC_F,'scx')
            NewChrom = feval(REC_F, parents, Dist, RecOpt);
        else
            NewChrom = feval(REC_F, parents, RecOpt);
        end
        NewChromSub=[NewChromSub; NewChrom];
      end
   end
end
