...
function NewChrom = order_cross(OldChrom, x_probability)
    ...
    if rand<x_probability
        ...
        if ~isempty(indexes_elements)
            if (cross_over_points(2)~=length(OldChrom(2,:))) && (max(indexes_elements)>cross_over_points(2))
                start_index = min(indexes_elements(find((indexes_elements>cross_over_points(2))==1)));
                indexes_elements = circshift(indexes_elements,[0,(length(indexes_elements)+1)-find(indexes_elements==start_index)]);
            end
        ...
        if ~isempty(indexes_elements)
            if (cross_over_points(2)~=length(OldChrom(1,:))) && (max(indexes_elements)>cross_over_points(2))
                start_index = min(indexes_elements(find((indexes_elements>cross_over_points(2))==1)));
                indexes_elements = circshift(indexes_elements,[0,(length(indexes_elements)+1)-find(indexes_elements==start_index)]);
            end
